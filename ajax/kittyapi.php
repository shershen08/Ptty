...
// Commands
    ...
    // Show a kitty!
        if(isset($in[1]) && is_numeric($in[1])){
            ...
            if(file_exists('../img/kitties/'.$kitty_id.'.png')){
                ...
                $url = $schema.$_SERVER['HTTP_HOST'].'/'.$_SERVER['SCRIPT_NAME'].'?kitty_id='.$kitty_id.'&'.time();
                if(isset($in[2])){
                // Check for callbacks
                    switch($in[2]){
                        case 'redirect':
                ...
                $out['out'] = 'That kitty ID dosen\'t exist.<br /><i>Hint</i>: Try a number from 01 to 10.';  
            }
        }else{
            $out['out'] = 'ID must be a number.<br>Usage: kittyget [kitty ID]';
        }
    ...
    // Put a kitty in the API
        ...
        if(count($kittys) >= 10){
            ...
            $help = '<p>Try using <a href="img/kitties/test/ktytest.png" terget="_blank">this image url</a> for example</p>';
            if( isset($in[1]) && filter_var($in[1], FILTER_VALIDATE_URL) !== false ){
                // Save the godamn image.
                $tmp = tempnam("/tmp", "tmp_image");
                if(file_put_contents($tmp, file_get_contents($in[1]))){

                    $img_info = getimagesize($tmp);

                    if(filesize($tmp) > 1024 && isset($img_info['mime']) && $img_info['mime'] == 'image/png'){
                        ...
                        if(imagepng($thumb, '../img/kitties/'.$kitty_id.'.png', 8)){
                            $out['out']  = 'URL is vallid. Kitty added with ID '.$kitty_id.'.';
                            $out['out'] .= '</br><img src="img/kitties/'.$kitty_id.'.png?'.time().'" alt=" kitty '.$kitty_id.'"/>';
                        }else{
                        ...
                        // delete file.
                        unlink($tmp);
                        $out['out']  = '<p>Only kitties (smaller than 1Mb and in PNG format) please.</p>';
                        $out['out'] .= $help;
                    }
                }else{
                ...
                $out['out'] = '<p>Invalid URL.<br>Usage: kittyput [URL to image]</p>';
                $out['out'] .= $help;
            }   
        }
    ...
    // Remove a kitty
        if(isset($in[1]) && is_numeric($in[1])){
            ...
            $out['out'] = 'ID must be a number.<br>Usage: kittydel [kitty ID]';
        }
